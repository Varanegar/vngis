...
BEGIN
	...
	BEGIN
		...
		SELECT	customer.Id, [Title] AS [Desc],
			Longitude, Latitude,
			CASE 
			WHEN CustomerArea.AreaId = null THEN 0
			WHEN (@RoutId = null) AND CustomerArea.AreaId <> null THEN 1
			WHEN (@RoutId <> null) AND CustomerArea.AreaId = @RoutId THEN 1
			WHEN (@RoutId <> null) AND CustomerArea.AreaId <> @RoutId THEN 2
		...
		WHERE (cPoint.STIntersects(@polygon) = 1) and
		(
			(@RoutId = null) or
			(
				(@RoutId <> null) AND
				...
				or
					(	(@ShowCustOtherRout = 1) AND (CustomerArea.AreaId <> null) )
				or
					(	(@ShowCustEithoutRout = 1) AND (CustomerArea.AreaId  = null) )
				)
			)
		)
